<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 px-6 lg:px-0">
                @forelse ($pinjamanAktif as $pinjaman)
                    <div class="bg-white overflow-hidden shadow-sm rounded-lg p-6 flex flex-col justify-between border-l-4 {{ $pinjaman->sisa_hutang <= 0 ? 'border-green-500' : 'border-indigo-500' }}">
                        <div>
                            ...
                            <p class="text-sm text-gray-600 flex justify-between">
                                ...
                                <span class="font-bold">Rp {{ number_format($pinjaman->total_tagihan, 0, ',', '.') }}</span>
                            </p>
                            ...
                            @else
                                <p class="mt-2 text-sm text-red-600 flex justify-between">
                                    <span>Sisa Hutang:</span>
                                    <span class="font-bold text-lg">Rp {{ number_format($pinjaman->sisa_hutang, 0, ',', '.') }}</span>
                                </p>
                            @endif
                ...
                @empty
                    <div class="md:col-span-2 lg:col-span-3 text-center py-10 bg-white rounded-lg shadow-sm">
                        ...
                        <p class="mt-1 text-sm text-gray-500">Semua nasabah Anda bersih dari hutang atau belum mengajukan.</p>
                    </div>
                @endforelse
            </div>
        ...
        <div class="relative p-6 bg-white w-full max-w-md m-auto rounded-lg shadow-2xl">
            ...
            <form id="paymentForm" method="POST" action="{{ route('pembayaran.store') }}" enctype="multipart/form-data">
                ...
                <div class="mt-6 flex justify-end gap-3">
                     <button type="button" onclick="closePaymentModal()" class="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300 font-medium">
                        Batal
                    </button>
                    <button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 font-bold shadow">
                        Simpan Pembayaran
                    </button>
                </div>
            </form>
    ...
    <script>
        ...
        function openPaymentModal(pinjamanId, anggotaNama, sisaHutang) {
            ...
            // Set max pembayaran agar tidak lebih dari sisa hutang (opsional, tapi bagus untuk UX)
            inputJumlahBayar.max = sisaHutang; 
            inputJumlahBayar.value = ''; // Kosongkan input lama
